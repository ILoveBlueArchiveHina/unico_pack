...
class MultiMarkerLanding(Node):
    def __init__(self):
        ...
        # 系統狀態
        self.detected_markers = {}  # 儲存偵測到的標記 {id: position}
        ...
        # 降落參數
        self.MARKER_IDS = [0, 1, 2, 3]  # 4個標記ID
        self.MARKER_SIZE = 0.10  # 10公分
        self.MIN_MARKERS_REQUIRED = 2  # 至少要看到2個標記
        self.LANDING_ALTITUDE_THRESHOLD = 0.3  # 30公分內開始最終降落
        self.DESCENT_SPEED = -0.2  # 下降速度 m/s
    ...
    def markers_callback(self, msg):
        ...
        # 計算多標記幾何中心
        if len(self.detected_markers) >= self.MIN_MARKERS_REQUIRED:
            ...
            if len(self.detected_markers) != self.last_marker_count:
                self.get_logger().info(
                    f'Detected {len(self.detected_markers)} markers '
                )
                self.last_marker_count = len(self.detected_markers)
        else:
            self.get_logger().warn(
                f'Only {len(self.detected_markers)} markers detected, '
    ...
    def control_loop(self):
        ...
        #     # 首次對齊，記錄時間
        #     if self.aligned_time is None:
        #         self.aligned_time = self.current_time
        #         self.get_logger().info(f'開始對齊 (偏差: xy={xy_offset:.3f}m, yaw={yaw_offset:.1f}°)')
            
        #     # 檢查是否持續對齊足夠時間
        #     if self.current_time - self.aligned_time >= self.ALIGNMENT_HOLD_TIME:
        ...
        # Yaw 角度對齊控制（讓 marker 的 yaw 趨近於 0）
        # center['yaw'] 是 marker 的平均 yaw，當為 0 時無需旋轉
